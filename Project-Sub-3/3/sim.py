...
class PyBulletSim:
    ...
    def move_tool(self, position, orientation, speed=0.03):
        ...
        target_joint_state = p.calculateInverseKinematics(
            ...
            targetPosition=position, # the target position of the end-effector link
            targetOrientation=orientation, # the target orientation of the end-effector link
            maxNumIterations=100, # the maximum number of iterations for Inverse Kinematics
            residualThreshold=1e-5)  # the residual threshold for Inverse Kinematics convergence

        # move the robot's joints to the target joint configuration using self.move_joints function
    ...
    def get_grasp_position_angle(self, object_id):
        ...
        # Get position and orientation of the object (represented by object_id) in the simulation
        object_position, object_orientation = p.getBasePositionAndOrientation(object_id)

        # Convert the position of the object to a numpy array and save it as the gripper's position
        position = np.array(object_position)

        # Get the euler angles from the quaternion orientation of the object
        # save the yaw angle as the gripper's grasp angle
        _, _, grasp_angle = p.getEulerFromQuaternion(object_orientation)
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        self.move_tool(pre_grasp_position_over_bin, gripper_orientation, speed=0.01)

        # 3. Move gripper to pre_grasp_position_over_object
        self.move_tool(pre_grasp_position_over_object, gripper_orientation, speed=0.01)

        # 4. Move gripper to grasp_position
        self.move_tool(grasp_position, gripper_orientation, speed=0.01)

        # 5. Close gripper
        self.close_gripper()

        # 6. Move gripper to post_grasp_position
        self.move_tool(post_grasp_position, gripper_orientation, speed=0.01)

        # 7. Move robot to robot_home_configuration
        self.robot_go_home(speed=0.1)
