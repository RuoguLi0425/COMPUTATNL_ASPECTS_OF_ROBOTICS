...
class Camera(object):
    ...
    def compute_camera_matrix(self):
        ...
        FocalX = FocalY = self.focal_length
        CenterX = self.image_size[1] / 2
        CenterY = self.image_size[0] / 2
        intrinsic_matrix = np.eye(3)
        intrinsic_matrix[0, 0] = FocalX
        intrinsic_matrix[1, 1] = FocalY
        intrinsic_matrix[0, 2] = CenterX
        intrinsic_matrix[1, 2] = CenterY

        Imageratio = self.image_size[1] / self.image_size[0]
        projection_matrix = p.computeProjectionMatrixFOV(fov=self.fov_width,
                                                         aspect=Imageratio,
                                                         nearVal=self.near,
                                                         farVal=self.far)
        ## Computes the camera's projection_matrix, which is used to project 3D space points onto the 2D image plane.
